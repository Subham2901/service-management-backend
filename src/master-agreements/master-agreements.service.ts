...
// Latest MASTER AGREEMENTS SERVICE
/* import { Injectable, Logger, NotFoundException, HttpException, HttpStatus } from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import { MasterAgreement } from './schemas/master-agreement.schema';
 ...
 */
  ...
  /* async fetchAndStoreAgreementDetails(agreementId: number): Promise<any> {
    ...
    // Fetch from external API if not found in DB
    if (!rawDetails.length) {
      ...
      try {
        ...
        const validDetails = externalDetails.flatMap((domain) =>
          domain.roleDetails.map((roleDetail) => ({
            ...
            technologyLevel: roleDetail.technologyLevel,
          })),
        );
    ...
    return this.groupDetailsByDomain(rawDetails);
  }

  // Helper method to group details by domain
  private groupDetailsByDomain(details: any[]): any[] {
...
export class MasterAgreementsService {
  ...
  async syncMasterAgreementsPeriodically() {
    this.logger.log('Starting periodic sync for master agreements and details');
    try {
      await this.fetchAndStoreAll();
      this.logger.log('Successfully completed periodic sync');
    } catch (error) {
      this.logger.error('Error during periodic sync', error.stack);
    }
  }

  async fetchAndStoreAll(): Promise<{ agreements: MasterAgreement[]; details: any[] }> {
    this.logger.log('Starting full sync for master agreements and details');
    try {
      const agreements = await this.fetchAndStoreMasterAgreements();
      const detailsPromises = agreements.map((agreement) =>
        this.fetchAndStoreAgreementDetails(agreement.agreementId),
      );
      const details = await Promise.all(detailsPromises);
      this.logger.log('Successfully synced all master agreements and details');
      return { agreements, details };
    } catch (error) {
      this.logger.error('Error during full sync', error.stack);
      throw new HttpException('Failed to sync master agreements and details', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
